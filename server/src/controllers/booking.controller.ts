...
export async function getPassengerBookings(req: AuthRequest, res: Response): Promise<void> {
  try {
    if (!req.user) {
      res.status(401).json({ message: "Authentication required." });
      return;
    ...
    const bookings = await Booking.find({ passengerId: req.user._id }).sort({ createdAt: -1 });
...
export async function getDriverRidesBookings(req: AuthRequest, res: Response): Promise<void> {
  try {
    if (!req.user) {
      res.status(401).json({ message: "Authentication required." });
      return;
    ...
    const rides = await Ride.find({ driverId: req.user._id }, "_id");
