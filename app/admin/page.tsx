  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-200">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {/* Success Rate Gauge */}
        <Card className="border-neutral-800 bg-neutral-900/40 col-span-1 flex flex-col justify-between">
          <CardHeader>
            ...
            <div className="relative flex items-center justify-center size-32">
              <svg className="size-full -rotate-90">
                ...
                <circle
                  ...
                  strokeDashoffset={2 * Math.PI * 54 * (1 - (stats?.bookingSuccessRate || 0) / 100)}
                  strokeLinecap="round"
                />
              </svg>
              <div className="absolute flex flex-col items-center">
                <span className="text-3xl font-black text-neutral-100">{stats?.bookingSuccessRate || 0}%</span>
                <span className="text-[10px] text-neutral-500 uppercase tracking-wider font-bold">Success</span>
              </div>
            </div>
            <p className="text-xs text-neutral-400 text-center mt-4 px-2">
              Ratio of bookings confirmed/accepted by drivers over total bookings.
            </p>
          </CardContent>
        </Card>

        {/* Booking Status Distribution */}
        <Card className="border-neutral-800 bg-neutral-900/40 col-span-1">
          <CardHeader>
            <CardTitle className="text-sm font-semibold uppercase tracking-wider text-neutral-400">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-medium">
                <span className="text-neutral-400">Accepted Bookings</span>
                <span className="text-emerald-400 font-bold">{stats?.acceptedBookings || 0}</span>
              </div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-medium">
                <span className="text-neutral-400">Pending Bookings</span>
                <span className="text-amber-400 font-bold">{stats?.activeRequests || 0}</span>
              </div>
              <div className="h-2 w-full rounded-full bg-neutral-800 overflow-hidden">
                <div
                  className="h-full bg-amber-500 rounded-full"
                  style={{ width: `${stats?.totalBookings ? ((stats.activeRequests / stats.totalBookings) * 100) : 0}%` }}
                />
              </div>
            </div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-medium">
                <span className="text-neutral-400">Rejected Bookings</span>
                <span className="text-red-400 font-bold">{stats?.rejectedBookings || 0}</span>
              </div>
              <div className="h-2 w-full rounded-full bg-neutral-800 overflow-hidden">
                <div
                  className="h-full bg-red-500 rounded-full"
                  style={{ width: `${stats?.totalBookings ? ((stats.rejectedBookings / stats.totalBookings) * 100) : 0}%` }}
                />
              </div>
            </div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-medium">
                <span className="text-neutral-400">Cancelled Bookings</span>
                <span className="text-neutral-500 font-bold">{stats?.cancelledBookings || 0}</span>
              </div>
              <div className="h-2 w-full rounded-full bg-neutral-800 overflow-hidden">
                <div
        ...
        <Card className="border-neutral-800 bg-neutral-900/40 col-span-1">
          <CardHeader>
            <CardTitle className="text-sm font-semibold uppercase tracking-wider text-neutral-400">
              Ride Request Analytics
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex justify-between items-center border-b border-neutral-800 pb-2.5">
              <div>
                <div className="text-xs font-semibold text-neutral-400">Total Custom Requests</div>
                <div className="text-lg font-bold text-neutral-200 mt-0.5">{stats?.totalRideRequests || 0}</div>
              </div>
              <div className="text-[10px] text-neutral-500 font-bold uppercase">Posted by Passengers</div>
            </div>

            <div className="flex justify-between items-center border-b border-neutral-800 pb-2.5">
              <div>
                <div className="text-xs font-semibold text-neutral-400">Pending Requests</div>
                <div className="text-lg font-bold text-amber-400 mt-0.5">{stats?.pendingRideRequests || 0}</div>
              </div>
              <div className="text-[10px] text-neutral-500 font-bold uppercase">Needs Driver Pickup</div>
            </div>

            <div className="flex justify-between items-center">
              <div>
                <div className="text-xs font-semibold text-neutral-400">Accepted Requests</div>
                <div className="text-lg font-bold text-emerald-400 mt-0.5">{stats?.acceptedRideRequests || 0}</div>
              </div>
              <div className="text-[10px] text-neutral-500 font-bold uppercase">Matched & Scheduled</div>
            </div>
          </CardContent>
        </Card>
      </div>
